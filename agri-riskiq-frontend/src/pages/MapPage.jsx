...
export default function MapPage() {
  ...
  const handleCreated = (e) => {
    ...
    const color = score >= 70 ? "#E53935" : score >= 40 ? "#FB8C00" : "#34A853"; // red, orange, green
    ...
    const popup = L.popup().setContent(
      `<div style="font-size:13px;">
        ...
        <span style="color:${color};font-weight:bold;">${score >= 70 ? "High Risk" : score >= 40 ? "Moderate" : "Low Risk"}</span>
      </div>`
    );
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        <h3 className="text-sm font-semibold text-gray-700 mb-2">AI Risk Summary</h3>
        {zones.length === 0 ? (
          <p className="text-gray-500 text-sm">No zones drawn yet. Draw a polygon or circle on the map.</p>
        ) : (
          <ul className="space-y-1 text-sm text-gray-700">
            {zones.map((z) => (
              <li key={z.id} className="flex justify-between border-b border-gray-100 pb-1">
                <span>
